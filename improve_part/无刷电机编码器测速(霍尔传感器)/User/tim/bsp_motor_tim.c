...
static void TIM_Mode_Config(void)
{
  ...
  //当定时器从0计数到PWM_PERIOD_COUNT，即为6000次，为一个定时周期
...
static void update_motor_speed(uint8_t dir_in, uint32_t time)
{
  ...
  /* 计算速度：
     电机每转一圈共用12个脉冲，(1.0/(90000000.0/128.0)为计数器的周期，(1.0/(90000000.0/128.0) * time)为时间长。
  */
  ...
  {
    f = (1.0f / (90000000.0f / HALL_PRESCALER_COUNT) * time);
