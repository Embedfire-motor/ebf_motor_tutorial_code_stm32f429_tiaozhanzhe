...
#define BASIC_PRESCALER_COUNT   (9000)
...
/* 以下两宏仅适用于定时器时钟源TIMxCLK=90MHz，预分频器为：9000-1 的情况 */
#define SET_BASIC_TIM_PERIOD(T)     __HAL_TIM_SET_AUTORELOAD(&TIM_PIDHandle, (T) - 1)    // 设置定时器的周期（1~1000ms）
